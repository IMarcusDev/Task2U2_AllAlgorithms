...
{
    internal class Sutherland_Hodgman_Algorithm
    {
        public static PointF[] clippingSutherlandHodgmanAlgorithm(List<PointF> points, PointF[] pointsCanvas)
        {
            List<PointF> clipedPoints = new List<PointF>();
            if (points.Count < 1) { return clipedPoints.ToArray(); }
